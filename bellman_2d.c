...
int main(int argc, char *argv[])
{
    ...
    if (n_iteration != -1)
    {
        ...
        for (int i = 0; i < 10; i++)
        {
            ...
            sprintf(filename, "path_from_(%d,%d)_to_(%d,%d)_1.txt", SOURCE_X, SOURCE_Y, dest_x, dest_y);
            if (USE_RANDOM_WEIGHTS)
                sprintf(filename, "path_from_(%d,%d)_to_(%d,%d)_random.txt", SOURCE_X, SOURCE_Y, dest_x, dest_y);
