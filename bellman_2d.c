...
int write_path_to_file(const int DESTINATIONS[10][2], int SOURCE_X, int SOURCE_Y, int X_DIM, int Y_DIM, int *P, int USE_RANDOM_WEIGHTS)
{
    for (int i = 0; i < 10; i++)
    {
        ...
        sprintf(filename, "shortest_paths/2d/path_from_(%d,%d)_to_(%d,%d)_1.txt", SOURCE_X, SOURCE_Y, dest_x, dest_y);
        if (USE_RANDOM_WEIGHTS)
            sprintf(filename, "shortest_paths/2d/path_from_(%d,%d)_to_(%d,%d)_random.txt", SOURCE_X, SOURCE_Y, dest_x, dest_y);
