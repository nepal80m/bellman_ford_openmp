...
int main(int argc, char *argv[])
{
    ...
    if (n_iteration != -1)
    {
        ...
        for (int i = 0; i < 10; i++)
        {
            ...
            sprintf(filename, "path_from_(%d,%d,%d)_to_(%d,%d,%d)_1_parallel.txt", SOURCE_X, SOURCE_Y, SOURCE_Z, dest_x, dest_y, dest_z);
            if (USE_RANDOM_WEIGHTS)
                sprintf(filename, "path_from_(%d,%d,%d)_to_(%d,%d,%d)_random_parallel.txt", SOURCE_X, SOURCE_Y, SOURCE_Z, dest_x, dest_y, dest_z);
